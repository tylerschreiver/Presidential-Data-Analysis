...
async function readCSVFiles() {
  try {
    ...
    const stateLossStreaks = stateDataWins.map(stateData => {
      ...
      return {
        ...stateData,
        maxLossLength: maxLosses,
        endLossIndex: endIndex,
        lossStreakStartingYear: years[endIndex - maxLosses + 1],
        lossStreakEndingYear: years[endIndex],
      }
    ...
    stateLossStreaks.forEach(stateData => {
      if (stateData.maxLossLength > maxLosses) {
        maxLosses = stateData.maxLossLength
      }
    })

    const maxLs = stateLossStreaks.filter(data => data.maxLossLength === maxLosses)
    ...
    const stateWinLossStreaks = stateLossStreaks.map(stateData => {
      ...
      return {
        ...stateData,
        maxWinLength: maxWines,
        endWinIndex: endIndex,
        winStreakStartingYear: years[endIndex - maxLosses + 1],
        winStreakEndingYear: years[endIndex],
      }
    }).sort((a, b) => b.maxWinLength - a.maxWinLength)


    let maxWins = 0
    stateWinLossStreaks.forEach(stateData => {
      if (stateData.maxWinLength > maxWins) {
        maxWins = stateData.maxWinLength
      }
    })

    const maxWs = stateWinLossStreaks.filter(data => data.maxWinLength === maxWins)
